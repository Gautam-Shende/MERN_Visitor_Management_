...
const Visitors = () => {
  ...
  const columns = [
    ...
    // },
    {
      ...
      accessor: 'status',
      cell: (row) => (
         <StatusBadge status={row.status} type="visitor" size="sm" /> ),
      sortable: true
    },
    {
      header: 'Visit Date',
  ...
  const statusCounts = {
    ...
    requested: visitors.filter(v => v.status === 'requested').length,
  ...
  const status = 
    [
      { label: 'All', value: 'all', count: statusCounts.all, color: 'bg-gray-200 text-gray-600' },
      { label: 'Pending', value: 'pending', count: statusCounts.pending, color: 'bg-amber-100 text-amber-600' },
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <button
            ...
            className={`
              ...
              ${filter === status.value ? 'border-[#2463eb] ring-2 ring-[#2463eb] ring-opacity-20' 
