...
export const generatePass = async (req, res) => {
  ...
  try {
    ...
    if (err.message === MESSAGES.APPOINTMENT_NOT_FOUND) return res.status(HTTP_STATUS.NOT_FOUND).json({ message: err.message })
    if (err.message === MESSAGES.APPOINTMENT_NOT_APPROVED) return res.status(HTTP_STATUS.NOT_FOUND).json({ message: MESSAGES.APPOINTMENT_NOT_APPROVED })
    if (err.message === MESSAGES.PASS_ALREADY_EXISTS) return res.status(HTTP_STATUS.CONFLICT).json({ message: MESSAGES.PASS_ALREADY_EXISTS })
...
export const scanPass = async (req, res) => {
  ...
  try {
    ...
    // console.log({error: MESSAGES.APPOINTMENT_NOT_REQUESTED})
...
export const getCheckLogsByPassId = async (req, res) => {
  ...
  try {
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ message: MESSAGES.PASS_NOT_FETCHED || MESSAGES.SERVER_ERROR })
  }
}
