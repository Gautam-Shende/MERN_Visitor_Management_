...
import { getRecentVisitors } from '../../services/visitorService'
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const [statsData, visitorsData, appointmentsData, passesData] = await Promise.all([
        getDashboardStats(),
        // getRecentVisitors(),
        // getAppointments(appointment._id),
  ...
  const weeklyData = [
    // { day: 'Mon', checkIns: 12, checkOuts: 8, visitors: 15, appointments: 10 },
    { day: 'Mon', checkIns: 12, appointments: 10 },
   ...
   const todayAnalytics = stats ? [
    ...
    {
      name: "Check-ins",
      value: stats.checkLogs?.todayCheckIns || 0,
      color: "#f59e0b" 
    },
    {
      name: "Check-outs",
      value: stats.checkLogs?.todayCheckOuts || 0,
      color: "#ef4444" 
    },
  ] : []
  ...
  const getStatusBadge = (status) => {
    const statusFilter = {
      ...
      pending: { bg: 'bg-yellow-100', text: 'text-yellow-800', icon: FaSpinner },
      rejected: { bg: 'bg-red-100', text: 'text-red-800', icon: FaTimesCircle },
      active: { bg: 'bg-green-100', text: 'text-green-800', icon: FaCheckCircle },
      inside: { bg: 'bg-blue-100', text: 'text-blue-800', icon: FaSignInAlt },
      expired: { bg: 'bg-gray-100', text: 'text-gray-800', icon: FaTimesCircle },
    }

    const filter = statusFilter[status] || statusFilter.pending
   
    const Icon = filter.icon
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${filter.bg} ${filter.text}`}>
        <Icon size={10} /> {status}
      </span>
    )
  }
  ...
  const summaryCards = [
    ...
    // { title: 'Total Appointmetn', value: stats?.visitors?.total || 0, icon: FaCalender, color: 'bg-blue-500',  },
    { title: 'Total Visitors', value: stats?.visitors?.total || 0, icon: FaUsers, color: 'bg-blue-500',  },
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      {/* <div className="flex xl:flex-3 lg:flex-2 sm:flex-col gap-4"> */}
              ...
              <div>
                ...
                {/* <div className="flex items-center mt-1">
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                    card.change.includes('+') ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
                  }`}>
                    {card.change}
                  </span>
                  <span className="text-xs text-gray-400 ml-1">{card.period}</span>
                </div> */}
              </div>
    ...
    {stats && todayAnalytics.length > 0 ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="p-6 text-center">
          <h3 className="text-md font-semibold text-gray-800 mb-4">Visitor Status</h3>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie 
        ...
        <Card className="p-6 text-center">
          <h3 className="text-md font-semibold text-gray-800 mb-4">Appointment Status</h3>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie 
        ...
        {/* Pass Status Chart */}
        <Card className="p-6 text-center">
          <h3 className="text-md font-semibold text-gray-800 mb-4">Pass Status</h3>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie 
