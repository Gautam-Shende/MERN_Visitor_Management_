...
const VisitorDetails = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    const statusConfig = {
      approved: { bg: 'bg-[#e6f7ee]', text: 'text-[#0b8a4f]', border: 'border-[#b0e3cd]', label: 'Approved' },
      pending: { bg: 'bg-[#fef5e6]', text: 'text-[#b45b0a]', border: 'border-[#fed7a5]', label: 'Pending' },
      rejected: { bg: 'bg-[#fee9e7]', text: 'text-[#cc2e2e]', border: 'border-[#fccac4]', label: 'Rejected' },
      checked_in: { bg: 'bg-[#e8f0fe]', text: 'text-[#2463eb]', border: 'border-[#b8d1fc]', label: 'Checked In' },
      checked_out: { bg: 'bg-[#f0f2f5]', text: 'text-[#4a5f73]', border: 'border-[#d0d9e6]', label: 'Checked Out' },
    }

    // const pagestatus = statusConfig[status = "pending"]
    const pagestatus = statusConfig[status] || statusConfig.pending;
  
    return (
      <span className={`px-3 py-1.5 rounded-lg text-xs font-medium border ${pagestatus.bg} ${pagestatus.text} ${pagestatus.border}`}>
        {pagestatus.label}
      </span>
    );
  };
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-[#0b1e3c]">
                    {new Date(visitor.createdAt || Date.now()).toLocaleDateString('en-US', {
                      weekday: 'long',
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
